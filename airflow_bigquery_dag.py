...
with DAG(
    ...
    with TaskGroup("load_data") as load_data:
        ...
        gcs_to_bq_merchants = GCSToBigQueryOperator(
            ...
            destination_project_dataset_table='vigilant-axis-460418-c4.walmart_dwh.merchants_tb',
        ...
        gcs_to_bq_walmart_sales = GCSToBigQueryOperator(
            ...
            destination_project_dataset_table='vigilant-axis-460418-c4.walmart_dwh.walmart_sales_stage',
            write_disposition='WRITE_TRUNCATE',
            source_format='NEWLINE_DELIMITED_JSON',
        )

    # Task 4: Merge (UPSERT) logic using BigQueryInsertJobOperator
    merge_walmart_sales = BigQueryInsertJobOperator(
        task_id='merge_walmart_sales',
        configuration={
            "query": {
                "query": """
                    MERGE `vigilant-axis-460418-c4.walmart_dwh.walmart_sales_tgt` T
                    USING (
                      SELECT 
                        S.sale_id, 
                        S.sale_date, 
                      ...
                      FROM `vigilant-axis-460418-c4.walmart_dwh.walmart_sales_stage` S
                      LEFT JOIN `vigilant-axis-460418-c4.walmart_dwh.merchants_tb` M
